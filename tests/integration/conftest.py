...
def approval_client(cognite_client: CogniteClient) -> CogniteClient:
    ...
    def create_dump(
        dct: dict[str, Union[CogniteResourceList, dict[str, Any]]]
    ) -> Callable[[], dict[str, Any]]:
        def dump() -> dict[str, Any]:
            # prepare a dump dict:
            # (nodes and edges need some special consideration)
            dumped: dict[str, list] = {key: [] for key in {*dct.keys()}}
            # dump:
            for key in sorted(dct):
                values = dct[key] or []
                for value in values:
                    dumped[key].append(
                        value.dump(camel_case=True)
            ...
            for key, values in list(dumped.items()):
                if not values:
                    ...
                    dumped[key] = sorted(
                        ...
                        key=lambda x: next(
                            x[k]
                            for k in ["externalId", "external_id", "name", "id"]
                            if k in x
                        ),
                    )
            return dumped
    ...
    with monkeypatch_cognite_client() as client:
        for api_instance, name, read_list_cls, write_list_cls in [
            ...
            (
                cognite_client.data_modeling.instances,
            ...
            (
                cognite_client.data_modeling.data_models,
            ...
            mock_api = create_mock_api(
                api_instance, read_list_cls, inputs, outputs, write_list_cls
            )
